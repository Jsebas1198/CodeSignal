...
function solution(n) {
  ...
  let minutes = n % 60;
  let stringHours = hours.toString().split("");
  let stringMinutes = minutes.toString().split("");
  arrayHours = stringHours.map((item) => {
    return parseInt(item, 10);
  });
  arrayMinutes = stringMinutes.map((item) => {
    return parseInt(item, 10);
  });

  return (
    arrayHours.reduce((a, b) => a + b) + arrayMinutes.reduce((a, b) => a + b)
  );
}
