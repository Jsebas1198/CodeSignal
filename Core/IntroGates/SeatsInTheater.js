...
// columns to your left. To gain some courage, you decide to calculate the number of such people and see
...
function solution(nCols, nRows, col, row) {
  ...
  for (let i = 0; i < nRows; i++) {
    matrix[i] = new Array(nCols).fill(1);
  }

  for (let i = 0; i < nRows; i++) {
    for (let j = 0; j < nCols; j++) {
      if (i >= nRows - row || j < col - 1) {
        matrix[i][j] = 0;
      }
      cont += matrix[i][j];
    }
